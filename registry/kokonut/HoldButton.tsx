...
interface HoldButtonProps
  ...
  holdDuration?: number;
  /** Called once after a completed pointer or keyboard hold. */
  onConfirm?: () => void;
}
...
export default function HoldButton({
  ...
  function handleHoldEnd() {
    ...
    setIsHolding(false);
    controls.stop();
    void controls.start({ width: "0%", transition: { duration: 0.1 } });
  }
  ...
  return (
    <Button
      ...
      disabled={disabled}
      onPointerDown={(event) => {
        if (event.button === 0) handleHoldStart();
      }}
    ...
    >
      ...
      <span className="relative z-10 flex w-full items-center justify-center gap-2">
        ...
        {confirmed ? "Confirmed" : isHolding ? "Keep holding…" : "Hold to confirm"}
      </span>
    </Button>
  );
}
