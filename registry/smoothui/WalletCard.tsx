  ...
  if (network === "mastercard") {
    return (
      ...
      >
        ...
        <path d="M20 5.1a8.5 8.5 0 0 0 0 13.8 8.5 8.5 0 0 0 0-13.8Z" fill="#ff5f00" />
      </svg>
    );
  }
...
const BalanceDisplay = ({
  ...
  const symbol =
    formatter.formatToParts(value).find((part) => part.type === "currency")?.value ?? "";
      ...
      <AnimatePresence initial={false} mode="popLayout">
        {characters.map((char, index) => (
          <motion.span
            ...
            initial={
              shouldReduceMotion ? { opacity: 1 } : { filter: `blur(${BLUR_AMOUNT}px)`, opacity: 0 }
            }
  ...
  return (
    <div className={cn("flex flex-col gap-5", className)}>
      ...
      <SmoothButton
        ...
        prefix={hidden ? <EyeOff aria-hidden="true" /> : <Eye aria-hidden="true" />}
        shape="pill"
        size="sm"
        variant="outline"
      >
