 ...
 * Source: https://github.com/educlopez/smoothui/blob/df0453c967224a91d460aa3bd1786c737c2c95bc/packages/smoothui/components/duration-picker/index.tsx
...
const decomposeDuration = (totalSeconds: number, units: DurationUnit[]): number[] => {
...
const getSegmentMax = (index: number, units: DurationUnit[], overallMax: number): number => {
  if (index === 0) {
...
const AnimatedDigit = ({ char, reduceMotion }: AnimatedDigitProps) => (
  <span className="relative inline-block w-[0.62em] text-center" style={{ height: DIGIT_HEIGHT }}>
    <AnimatePresence initial={false} mode="popLayout">
      <motion.span
        ...
        initial={reduceMotion ? { opacity: 1, y: 0 } : { opacity: 0, y: -DIGIT_ROLL_DISTANCE }}
        key={char}
        transition={
          reduceMotion ? { duration: 0 } : { bounce: 0.1, duration: 0.25, type: "spring" }
        }
      >
        {char}
      </motion.span>
...
const DurationPicker = ({
  ...
  const values = useMemo(() => decomposeDuration(value, sortedUnits), [value, sortedUnits]);
  ...
  const handlePointerMove = useCallback(
    (event: ReactPointerEvent<HTMLDivElement>) => {
      ...
      const deltaSteps = Math.round((event.clientX - drag.startX) / PX_PER_DRAG_STEP);
  ...
  return (
    <div className={cn("inline-flex flex-col gap-1.5", className)}>
      {label ? (
        <span className="font-medium text-muted-foreground text-xs" id={labelId}>
          {label}
        </span>
      ) : null}
      <div
