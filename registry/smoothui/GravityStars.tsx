 ...
 */
const paintSprites = (sprites: HTMLCanvasElement[], palette: Palette, tint: number) => {
  const last = Math.max(sprites.length - 1, 1);
  for (let level = 0; level < sprites.length; level++) {
    const ramp = level / last;
    const color: Rgb = [
      Math.round(mix(palette.base[0], mix(palette.cool[0], palette.warm[0], ramp), tint)),
      Math.round(mix(palette.base[1], mix(palette.cool[1], palette.warm[1], ramp), tint)),
      Math.round(mix(palette.base[2], mix(palette.cool[2], palette.warm[2], ramp), tint)),
    ];
...
const createStarController = (canvas: HTMLCanvasElement): StarController | null => {
  ...
  const applyPointer = (i: number, out: Float32Array) => {
    ...
    const strength =
      Math.min((settings.gravity * POINTER_ACCEL * pull) / (softened * softened), MAX_ACCEL) *
      fade *
      fade;
  ...
  const buildSegments = (cols: number, rows: number) => {
    ...
    const consider = (a: number, b: number) => {
      ...
      const closeness = (1 - Math.sqrt(d2) / settings.connectDistance) ** LINE_CLOSENESS_CURVE;
      const depthWeight = LINE_DEPTH_FLOOR + ((depth[a] + depth[b]) / 2) * LINE_DEPTH_GAIN;
      const level = Math.min(LINE_LEVELS - 1, Math.floor(closeness * depthWeight * LINE_LEVELS));
  ...
  const drawStars = () => {
    ...
    for (let i = 0; i < activeCount; i++) {
      ...
      const wave = still ? Math.sin(phase[i]) : Math.sin(elapsed * rate[i] + phase[i]);
      const brightness =
        (FAR_ALPHA_FLOOR + z * NEAR_ALPHA_GAIN) * (1 - twinkle * TWINKLE_DEPTH * (1 - wave));
      ...
      context.drawImage(sprites[tintLevel[i]], posX[i] - halo, posY[i] - halo, halo * 2, halo * 2);
    }
  ...
  return {
    ...
    setSettings: (next: StarSettings) => {
      ...
      const changedSprites =
        next.tint !== settings.tint || !samePalette(next.palette, settings.palette);
      settings = next;
      if (changedSprites) {
...
const GravityStars = ({
  ...
  useEffect(() => {
    ...
    const [base, cool, warm] = resolveCssColors([color, TINT_COOL, TINT_WARM], host);
    setPalette({
  ...
  useEffect(() => {
    ...
    const handleMove = (event: PointerEvent) => {
      const rect = canvas.getBoundingClientRect();
      controllerRef.current?.setPointer(event.clientX - rect.left, event.clientY - rect.top, true);
    };
    const handleLeave = () => {
