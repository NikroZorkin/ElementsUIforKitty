 ...
 * Source: https://github.com/educlopez/smoothui/blob/df0453c967224a91d460aa3bd1786c737c2c95bc/packages/smoothui/components/time-machine-stack/index.tsx
...
export default function TimeMachineStack({
  ...
  const currentIndex = clampIndex(isControlled ? (index as number) : internalIndex, lastIndex);
  ...
  const getPanelState = useCallback(
    (pos: number): PanelState => {
      ...
      return {
        ...
        transform: toTransform(-pos * offsetY, -pos * depth, 1 - pos * scaleStep),
      };
    },
  ...
  const reservedTop = shouldReduceMotion ? 0 : Math.max(visibleCount - 1, 0) * offsetY;

  const transition = shouldReduceMotion
    ? { duration: 0 }
    : { bounce: 0.1, duration: 0.25, type: "spring" as const };

  const exitState = direction === FORWARD ? getPanelState(-1) : getPanelState(visibleCount);
  ...
  return (
    ...
    <div className={cn("flex w-full flex-col items-center gap-4", className)}>
      ...
      <div
        aria-activedescendant={frontItem ? `${listboxId}-${frontItem.id}` : undefined}
              ...
              return (
                <motion.div
                  ...
                  exit={{
                    ...
                    transition: shouldReduceMotion ? { duration: 0 } : EXIT_ENTER_TRANSITION,
                  }}
                  ...
                  initial={direction === FORWARD ? getPanelState(visibleCount) : getPanelState(-1)}
                  key={item.id}
                  role="option"
