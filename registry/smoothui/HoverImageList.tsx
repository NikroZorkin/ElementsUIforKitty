 ...
 * Source: https://github.com/educlopez/smoothui/blob/df0453c967224a91d460aa3bd1786c737c2c95bc/packages/smoothui/components/hover-image-list/index.tsx
...
const HoverImageList = ({
  ...
  const isFixedPosition =
    shouldReduceMotion || !isHoverDevice || activationSource === "focus" || revealMode === "fixed";
      ...
      rafIdRef.current = requestAnimationFrame((now) => {
        ...
        if (follow) {
          ...
          if (lastPointerRef.current) {
            const dt = Math.min(now - lastPointerRef.current.t, MAX_FRAME_DELTA_MS);
            if (dt > 0) {
  ...
  const handleListBlur = (event: FocusEvent<HTMLDivElement>) => {
    if (activationSource === "focus" && !event.currentTarget.contains(event.relatedTarget)) {
      deactivateRow("focus");
    }
  };
  ...
  const renderRowContent = (item: HoverImageListItem): ReactNode => (
    <>
      <span className="flex flex-col gap-1">
        ...
        {item.meta ? <span className="text-muted-foreground text-sm">{item.meta}</span> : null}
      </span>
      <ArrowUpRight
        aria-hidden="true"
    ...
    >
      <ul className="flex flex-col">
        {items.map((item, index) => (
          <li className="border-foreground/10 border-b last:border-b-0" key={item.id}>
            {item.href ? (
              <a
