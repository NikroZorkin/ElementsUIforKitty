 ...
 * Source: https://github.com/educlopez/smoothui/blob/df0453c967224a91d460aa3bd1786c737c2c95bc/packages/smoothui/components/file-tree/index.tsx
...
const flattenVisible = (
  ...
  for (const node of nodes) {
    ...
    if (
      ...
      result.push(...flattenVisible(node.children, expandedSet, level + 1, node.id));
    }
  }
  return result;
};
...
const FileTreeRow = ({
  ...
  const defaultIcon = isFolder ? (
    ...
    <File className={isSelected ? "text-foreground" : "text-muted-foreground"} size={16} />
  );
  ...
  return (
    <div role="none">
      ...
      >
        ...
        <span className="flex shrink-0 items-center">{node.icon ?? defaultIcon}</span>
        <span
          className={cn("flex-1 truncate text-foreground text-sm", isSelected && "font-medium")}
        >
          {node.name}
        </span>
        {node.badge ? (
          <span className="ml-auto shrink-0 text-muted-foreground text-xs">{node.badge}</span>
        ) : null}
      </div>
...
const FileTree = ({
  ...
  const flatList = useMemo(() => flattenVisible(items, expandedSet, 1, null), [items, expandedSet]);
  ...
  const toggleExpanded = useCallback(
    (id: string, force?: boolean) => {
      ...
      setExpandedList(
        shouldOpen ? [...expandedList, id] : expandedList.filter((existing) => existing !== id),
      );
    },
  ...
  const handleKeyDown = useCallback(
    ...
    [flatIndexById, flatList, expandedSet, toggleExpanded, moveFocusTo, handleSelect, typeahead],
  );
