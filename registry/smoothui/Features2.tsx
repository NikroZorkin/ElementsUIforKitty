 ...
 * Source: https://github.com/educlopez/smoothui/blob/df0453c967224a91d460aa3bd1786c737c2c95bc/packages/smoothui/blocks/features/features-2/index.tsx
    ...
    <div className="flex h-full flex-col gap-4 rounded-xl border bg-background p-5">
      <div className="flex items-baseline justify-between gap-3">
        <div>
          ...
          <p className="font-semibold text-2xl text-foreground tabular-nums">$48,219</p>
        </div>
      ...
      <div aria-hidden="true" className="flex min-h-0 flex-1 items-end gap-1.5 sm:gap-2">
        {SERIES.map(({ id, value }, index) => (
          <motion.span
            ...
            style={{ height: `${value}%` }}
            transition={
              shouldReduceMotion ? { duration: 0 } : { ...SPRING, delay: index * BAR_STAGGER }
            }
            ...
            {features.map((feature, index) => (
              <motion.div
                className={cn(
                  ...
                  feature.accent ? "bg-foreground/5 ring-1 ring-foreground/10" : "bg-background",
                  feature.span,
                )}
