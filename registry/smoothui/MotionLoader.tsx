 ...
 * Source: https://github.com/educlopez/smoothui/blob/df0453c967224a91d460aa3bd1786c737c2c95bc/packages/smoothui/components/motion-loader/index.tsx
...
const delayedLoop = (duration: number, ease: [number, number, number, number], delay: number) => ({
  ...loop(duration, ease),
  delay,
});
    ...
    <motion.span
      animate={
        reduce
          ? undefined
          : {
              rotate: [-PENDULUM_SWING_DEG, PENDULUM_SWING_DEG, -PENDULUM_SWING_DEG],
            }
      }
      className="absolute inset-0"
...
const HourglassLoader = ({ reduce, size, speed }: LoaderVariantProps) => (
  <motion.svg
    animate={reduce ? undefined : { rotate: [0, 0, HALF_TURN, HALF_TURN, FULL_TURN] }}
...
const SquareSnakeLoader = ({ reduce, size, speed }: LoaderVariantProps) => {
  ...
  return (
    <>
      ...
      <motion.span
        animate={
          reduce ? undefined : { x: [0, travel, travel, 0, 0], y: [0, 0, travel, travel, 0] }
        }
  ...
  return (
    ...
    >
      {WAVE_IDS.map((id, index) => (
        <motion.span
          animate={reduce ? undefined : { scaleY: [WAVE_MIN_SCALE, 1, WAVE_MIN_SCALE] }}
          ...
          transition={
            reduce ? undefined : delayedLoop(CYCLE / speed, EASE_IN_OUT, index * WAVE_STAGGER)
          }
        />
      ))}
    </span>
    ...
    <>
      {RING_IDS.map((id, index) => (
        <motion.span
          ...
          transition={reduce ? undefined : delayedLoop(CYCLE / speed, LINEAR, index * RING_STAGGER)}
        />
      ))}
    </>
  );
...
const VARIANT_RENDERERS: Record<MotionLoaderVariant, (props: LoaderVariantProps) => ReactElement> =
  {
...
const MotionLoader = ({
  ...
  return (
    ...
    >
      <Renderer reduce={Boolean(shouldReduceMotion)} size={size} speed={safeSpeed} />
