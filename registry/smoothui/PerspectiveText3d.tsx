 ...
 * Source: https://github.com/educlopez/smoothui/blob/df0453c967224a91d460aa3bd1786c737c2c95bc/packages/smoothui/components/perspective-text-3d/index.tsx
...
const splitContent = (content: string, split: PerspectiveTextSplit): string[] => {
  if (split === "words") {
...
export default function PerspectiveText3D({
  ...
  const scrollRotateXRaw = useTransform(scrollYProgress, [0, 1], [rotateX, -rotateX]);
  const scrollRotateYRaw = useTransform(scrollYProgress, [0, 1], [-rotateY, rotateY]);
  ...
  const planeAnimate =
    driver === "pointer"
      ...
      : driver === "static"
        ? { rotateX, rotateY }
        : undefined;
  const planeInitial = driver === "scroll" ? undefined : { rotateX: 0, rotateY: 0 };

  return (
    <div className={cn("inline-block", className)} ref={wrapperRef} style={{ perspective }}>
      ...
      <motion.div
        ...
        style={{
          ...
          ...(driver === "scroll" ? { rotateX: scrollRotateX, rotateY: scrollRotateY } : {}),
        }}
