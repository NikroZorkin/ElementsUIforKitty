...
type TextSegment = { type: "text"; value: string } | { type: "token"; id: string };
  ...
  >
    <div className="flex items-center gap-3">
      ...
      <div className="min-w-0">
        <p className="truncate font-semibold text-foreground text-sm">{testimonial.name}</p>
        <p className="truncate text-muted-foreground text-xs">{testimonial.role}</p>
      </div>
    </div>
...
const InlineTestimonials = ({
  ...
  const measure = useCallback((id: string) => {
    ...
    const width = Math.min(CARD_WIDTH_PX, Math.max(containerRect.width - EDGE_PADDING_PX * 2, 0));
    const center = triggerRect.left - containerRect.left + triggerRect.width / 2;
    const maxLeft = Math.max(containerRect.width - width - EDGE_PADDING_PX, EDGE_PADDING_PX);
  ...
  const handleTriggerFocus = (event: FocusEvent<HTMLButtonElement>, id: string) => {
  ...
  const testimonialsById = new Map(testimonials.map((entry) => [entry.id, entry]));
  const segments = parseSegments(text);
  const activeTestimonial = activeOpenId ? testimonialsById.get(activeOpenId) : undefined;
            ...
            >
              ...
              <span
                className={cn(
                  ...
                  isOpen ? "border-brand" : "border-foreground/25 group-hover:border-foreground/50",
                )}
              >
                {testimonial.name}
              </span>
      ...
      <AnimatePresence>
        {activeTestimonial && position ? (
          ...
          <motion.div
            ...
            initial={
              shouldReduceMotion ? { opacity: 1 } : { opacity: 0, scale: 0.97, y: enterOffset }
            }
            ...
            style={{
              left: position.left,
              top: flipUp ? position.triggerTop - cardHeight : position.triggerBottom,
