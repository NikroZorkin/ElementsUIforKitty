 ...
 * Source: https://github.com/educlopez/smoothui/blob/df0453c967224a91d460aa3bd1786c737c2c95bc/packages/smoothui/components/animated-tags/index.tsx
          ...
          <div className="flex min-h-12 w-full flex-wrap items-center gap-1 rounded-xl border bg-background p-2">
            {selectedTag?.map((tag) => (
              <motion.button
                ...
                initial={
                  shouldReduceMotion ? { opacity: 1 } : { filter: "blur(4px)", opacity: 0, y: 20 }
                }
                key={tag}
                layout
                type="button"
      ...
      <AnimatePresence>
        <div className="flex flex-wrap items-center gap-1">
          {tags.map((tag) => (
            ...
            >
              {tag} <Plus className="flex items-center justify-center rounded-full" size={16} />
            </motion.button>
          ))}
        </div>
      </AnimatePresence>
