 ...
 */
export default function PhotoStack({ className, photos, onCycle }: PhotoStackProps) {
      ...
      {order.map((photoIndex, pos) => {
        ...
        return (
          <motion.div
            ...
            aria-label={isFront ? `${photo.name ?? photo.alt} — next` : undefined}
            ...
            transition={
              reduceMotion ? { duration: 0 } : { damping: 30, stiffness: 320, type: "spring" }
            }
          ...
          >
            ...
            {photo.name || photo.role ? (
              ...
              >
                {photo.name ? <span className="font-semibold text-sm">{photo.name}</span> : null}
                {photo.role ? <span className="text-xs opacity-85">{photo.role}</span> : null}
              </figcaption>
            ) : null}
            {pos > 0 && (
              <span
