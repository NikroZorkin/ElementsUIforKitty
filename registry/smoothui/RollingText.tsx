 ...
 * Source: https://github.com/educlopez/smoothui/blob/df0453c967224a91d460aa3bd1786c737c2c95bc/packages/smoothui/components/rolling-text/index.tsx
...
const MS = 1000;
const PERCENT = 100;
const DEFAULT_GLYPH_POOL = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
...
const buildCharacters = (
  ...
  return segmentGraphemes(text).map((char) => {
    ...
    const items = isWhitespace ? [char] : buildRamp(char, pool, rampLength, direction);
    return {
      isWhitespace,
      items,
      key: `${char}-${count}`,
...
export default function RollingText({
  ...
  const [playCount, setPlayCount] = useState(() => (trigger === "mount" ? 1 : 0));
  ...
  useEffect(() => {
    if (trigger === "manual" && playing && !wasPlayingRef.current && !shouldReduceMotion) {
      setPlayCount((count) => count + 1);
    }
  ...
  const characters = useMemo(
    ...
    [text, pool, rampLength, direction],
  );
  ...
  return (
    <span className={className} onPointerEnter={handlePointerEnter} ref={containerRef}>
