 ...
 * Source: https://github.com/educlopez/smoothui/blob/df0453c967224a91d460aa3bd1786c737c2c95bc/packages/smoothui/components/number-flow/index.tsx
  ...
  useEffect(() => {
    ...
    for (const digit of digits) {
      if (
        ...
        animations.push(...animateDigit(digit.previous, digit.next, value > prevValue));
      }
    }
    return () => {
  ...
  return (
    <div className={cn("flex flex-col items-center justify-center gap-8", className)}>
      <div className="flex items-center gap-2 rounded-xl border bg-background p-4">
        ...
        <div aria-hidden="true" className={cn("flex items-center gap-1", digitClassName)}>
          <div className={cn("relative h-16 w-12 overflow-hidden rounded-lg border bg-primary")}>
            <span
              className="absolute inset-0 flex items-center justify-center font-semibold text-2xl text-primary-foreground"
              ref={prevValueHunds}
