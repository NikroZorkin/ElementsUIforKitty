...
export default function OrbitalImageWheel({
  ...
  const currentRotation = isControlled ? (rotation as number) : internalRotation;
  ...
  useEffect(() => {
    ...
    const tick = (time: number) => {
      const dt = time - last;
      last = time;
      rotationRef.current = normalizeAngle(rotationRef.current + (autoRotateSpeed * dt) / 1000);
      commitRotation(rotationRef.current);
      raf = requestAnimationFrame(tick);
    };
    raf = requestAnimationFrame(tick);
    return () => cancelAnimationFrame(raf);
  }, [autoRotate, autoRotateSpeed, commitRotation, isPaused, items.length, shouldReduceMotion]);
  ...
  const handlePointerMove = useCallback(
    (event: ReactPointerEvent<HTMLDivElement>) => {
      ...
      velocityRef.current =
        velocityRef.current * (1 - VELOCITY_SMOOTHING) + (delta / dt) * VELOCITY_SMOOTHING;
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent<HTMLDivElement>) => {
      if (event.key === "ArrowRight") {
        ...
        snapTo(rotationRef.current - itemAngle, shouldReduceMotion ? 0 : STEP_DURATION);
  ...
  return (
    ...
    >
      ...
      <span aria-live="polite" className="sr-only">
        {highlightedItem ? `${highlightedItem.label ?? highlightedItem.alt} selected` : null}
      </span>
    </div>
  );
}
