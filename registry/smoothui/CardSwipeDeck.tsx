 ...
 * Source: https://github.com/educlopez/smoothui/blob/df0453c967224a91d460aa3bd1786c737c2c95bc/packages/smoothui/components/card-swipe-deck/index.tsx
...
const DeckCard = ({
  ...
  const rotate = useTransform(x, [-DRAG_RANGE, DRAG_RANGE], [-rotationFactor, rotationFactor]);
  ...
  useEffect(() => {
    ...
    const targetX = throwVector.direction === "left" ? -EXIT_DISTANCE : EXIT_DISTANCE;
...
export default function CardSwipeDeck({
  ...
  return (
    ...
    <div
      ...
      className={cn("relative h-[480px] w-[340px] max-w-full touch-pan-y outline-none", className)}
    ...
    >
      {visible.length === 0 ? (
        <div className="flex h-full w-full items-center justify-center rounded-[28px] border border-foreground/10 border-dashed">
          <p className="text-center text-muted-foreground text-sm">No more cards</p>
        </div>
      ) : (
