 ...
 * Source: https://github.com/educlopez/smoothui/blob/df0453c967224a91d460aa3bd1786c737c2c95bc/packages/smoothui/components/ai-approval/index.tsx
...
const AIApproval = ({
  ...
  return (
    <motion.div
      className={cn("w-full rounded-xl border border-border bg-background p-3.5", className)}
        ...
        <AnimatePresence initial={false} mode="popLayout">
          {chosen ? (
            ...
            >
              ...
              {chosen.detail ? (
                <span className="ml-auto text-xs opacity-70">{chosen.detail}</span>
              ) : null}
            </motion.div>
          ) : (
            <motion.div
              ...
              key="options"
              transition={shouldReduceMotion ? { duration: 0 } : { duration: 0.16, ease: EASE_OUT }}
            >
              {options.map((option, index) => (
                ...
                >
                  ...
                  {option.detail ? (
                    <span className="ml-auto text-muted-foreground text-xs">{option.detail}</span>
                  ) : null}
                </motion.button>
              ))}
            </motion.div>
