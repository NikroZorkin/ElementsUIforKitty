 ...
 * Source: https://github.com/educlopez/smoothui/blob/df0453c967224a91d460aa3bd1786c737c2c95bc/packages/smoothui/components/gravity-letters/index.tsx
...
const GravityLetters = ({
      ...
      for (let i = 0; i < physicsRef.current.length; i++) {
        ...
        if (phaseRef.current === "falling") {
          ...
          if (p.dy >= floorLimit) {
            ...
            p.vx *= 1 - f;
            p.angularVel *= 1 - f;
            if (Math.abs(p.vy) < REST_VELOCITY && Math.abs(p.vx) < REST_HORIZONTAL_VELOCITY) {
              p.vy = 0;
              p.vx = 0;
              p.angularVel = 0;
              p.settled = true;
          ...
          const aAngle = -SPRING_STIFFNESS * p.angle - SPRING_DAMPING * p.angularVel;
  ...
  }, [collapsed, shouldReduceMotion, startFalling, startReforming]);
  ...
  return (
    ...
    <div
      className={cn("relative isolate min-h-24 select-none overflow-hidden", className)}
    ...
    >
      ...
      <div aria-hidden="true" className="pointer-events-none invisible flex flex-wrap">
