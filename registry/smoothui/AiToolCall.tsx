 ...
 * Source: https://github.com/educlopez/smoothui/blob/df0453c967224a91d460aa3bd1786c737c2c95bc/packages/smoothui/components/ai-tool-call/index.tsx
...
const AIToolCallBadge = ({
  ...
  return (
    <span className="relative flex size-5 shrink-0 items-center justify-center text-muted-foreground">
      ...
      >
        <motion.g
          animate={shouldReduceMotion || !isRunning ? undefined : { rotate: 360 }}
        ...
        >
          <motion.circle
            animate={{
              ...
              strokeOpacity: isPending && !shouldReduceMotion ? [0.35, 1, 0.35] : 1,
            }}
            cx={BADGE_CENTER}
            cy={BADGE_CENTER}
            fill="none"
            ...
            strokeWidth={2}
            transition={{
              stroke: shouldReduceMotion ? { duration: 0 } : { duration: 0.25, ease: EASE_OUT },
        ...
        <AnimatePresence initial={false}>
          {status === "success" && (
            <motion.path
              ...
              fill="none"
              initial={
                shouldReduceMotion ? { opacity: 1, pathLength: 1 } : { opacity: 1, pathLength: 0 }
              }
              key="check"
              stroke={ringColor}
              strokeLinecap="round"
              ...
              strokeWidth={2.2}
              transition={shouldReduceMotion ? { duration: 0 } : { duration: 0.22, ease: EASE_OUT }}
            />
          )}
          {status === "error" &&
  ...
  return (
    ...
    >
      <button
        ...
        type="button"
      >
        <AIToolCallBadge shouldReduceMotion={Boolean(shouldReduceMotion)} status={status} />
        ...
        {summary ? <span className="shrink-0 text-muted-foreground text-xs">{summary}</span> : null}
            ...
            <div className="space-y-2 border-border border-t px-3 py-2.5 text-xs">
              {args ? (
                <div>
                  ...
                  <div className="overflow-x-auto font-mono text-foreground">{args}</div>
                </div>
              ) : null}
              {result ? (
                <div>
                  ...
                  <div className="overflow-x-auto text-foreground">{result}</div>
                </div>
              ) : null}
            </div>
          </motion.div>
