...
const applyParallax = (container: HTMLElement, normalizedX: number, normalizedY: number) => {
  const depthElements = container.querySelectorAll<HTMLElement>("[data-tilt-depth]");

  for (const element of depthElements) {
    const depth = Number.parseFloat(element.dataset.tiltDepth ?? "0");
    const boundedDepth = Number.isFinite(depth) ? Math.min(1, Math.max(0, depth)) : 0;
